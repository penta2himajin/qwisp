...
public struct GatedDeltaNetLayer {
    ...
    let inProjQKV: MLXArray   // [convDim, H]
    let inProjZ: MLXArray     // [valueDim, H]
    let inProjB: MLXArray     // [numVHeads, H]
    let inProjA: MLXArray     // [numVHeads, H]
    ...
    public init(numKHeads: Int, numVHeads: Int, headKDim: Int, headVDim: Int,
                ...
                inProjQKV: MLXArray, inProjZ: MLXArray, inProjB: MLXArray, inProjA: MLXArray,
                conv1dW: MLXArray, normWeight: MLXArray, outProjW: MLXArray,
        ...
        self.inProjB = inProjB; self.inProjA = inProjA
    ...
    public func callAsFunction(_ x: MLXArray) -> MLXArray {
        ...
        let qkv = GatedDeltaNetLayer.linear(x, inProjQKV)              // [B,S,convDim]
        let z = GatedDeltaNetLayer.linear(x, inProjZ).reshaped([B, S, numVHeads, headVDim])
        let b = GatedDeltaNetLayer.linear(x, inProjB)                 // [B,S,numVHeads]
        let a = GatedDeltaNetLayer.linear(x, inProjA)                 // [B,S,numVHeads]
...
public enum GatedDeltaNetLayerValidation {
    public static func run(refPath: String) throws -> String {
        ...
        let layer = GatedDeltaNetLayer(
            ...
            inProjQKV: qkv, inProjZ: z, inProjB: pb, inProjA: pa,
            conv1dW: cw, normWeight: nw, outProjW: ow, aLog: aLog, dtBias: dtBias)
        let out = layer(x)
        out.eval()
