...
public enum RawVerifyTests {
    ...
    public static func runAll() -> String {
        ...
        // ── LMHEAD2 tests (G1 gate, notes/05-lmhead-margin-cert-spec.md §6) ──────

        // Test 32 (G1-soundness): lmhead2_cert_soundness
        //
        // Property: for every row where cert=true, tokensOut[m] == 4-bit reference argmax (bit-exact).
        //
        // Method:
        //   - Draw random W [V=4096, K=2048] f16, quantize to 4-bit.
        //   - Call buildLMHead2 → get 2-bit copy + rowE.
        //   - Run 10 batches of 1000 hidden rows each (= 10,000 total rows).
        //     Every 4th batch is adversarial: h is scaled down by 0.01 to produce near-tie logits
        //     (tight margin → stress-tests the ε slack in the cert condition).
        //   - For each certified row, compute the 4-bit reference argmax via qmmRows (bit-exact
        //     per test qmm4_rows_bitexact) and assert exact token match.
        //   - ANY violation → FAIL.  cert-rate is printed as informational.
        //
        // RED (stub): fails at buildLMHead2 → "not implemented".
        run("lmhead2_cert_soundness") {
            let V = 4096, K = 2048
            let wf = MLXRandom.normal([V, K]).asType(.float16)
            let (wq4, sc4, bi4opt) = MLX.quantized(wf, groupSize: 64, bits: 4, mode: .affine)
            guard let bi4 = bi4opt else { return (false, "4-bit biases nil") }
            MLX.eval([wq4, sc4, bi4])
            guard let lh2 = RawFusedVerify.buildLMHead2(w4: wq4, s4: sc4, b4: bi4, V: V, K: K)
            else { return (false, "not implemented: buildLMHead2") }
            let (wq2, sc2, bi2, rowE) = lh2
            MLX.eval([wq2, sc2, bi2, rowE])

            let batchSize = 1000, numBatches = 10   // >= 10,000 rows total
            var certCount = 0, violations = 0

            for batch in 0..<numBatches {
                // Every 4th batch: scale h down 100x → near-tie logits (adversarial for cert margin)
                let scaleFactor: Float = (batch % 4 == 3) ? 0.01 : 1.0
                let hRaw = MLXRandom.normal([batchSize, K]).asType(.float16)
                let h: MLXArray = (scaleFactor == 1.0) ? hRaw
                    : (hRaw.asType(.float32) * MLXArray(scaleFactor)).asType(.float16)
                h.eval()

                guard let res = RawFusedVerify.lmhead2CertStep(
                    x: h, w4: wq4, s4: sc4, b4: bi4,
                    w2: wq2, s2: sc2, b2: bi2,
                    rowE: rowE, M: batchSize, K: K, V: V)
                else { return (false, "not implemented: lmhead2CertStep batch=\(batch)") }

                // Reference argmax for the whole batch (one batched call, bit-exact per test 1)
                guard let allLogits4 = RawMetalForward.qmmRows(
                    h, wq4, scales: sc4, biases: bi4, M: batchSize, K: K, N: V)
                else { return (false, "ref qmmRows nil batch=\(batch)") }
                allLogits4.eval()
                let logFlat = allLogits4.reshaped([-1]).asType(.float32).asArray(Float.self)

                for m in 0..<batchSize where res.cert[m] {
                    certCount += 1
                    let base = m * V
                    var refTok = 0; var best = logFlat[base]
                    for v in 1..<V { if logFlat[base + v] > best { best = logFlat[base + v]; refTok = v } }
                    if res.tokens[m] != refTok { violations += 1 }
                }
            }

            let certRate = Double(certCount) / Double(batchSize * numBatches) * 100.0
            if violations > 0 {
                return (false,
                    "cert soundness violation: \(violations)/\(certCount) cert rows disagree with " +
                    "4-bit argmax (cert-rate \(String(format: "%.1f", certRate))%)")
            }
            return (true,
                "lmhead2 cert-rate: \(String(format: "%.1f", certRate))% " +
                "(\(certCount)/\(batchSize * numBatches) rows certified, 0 violations)")
        }

        // Test 33 (G1-fallback): lmhead2_fallback_exact
        //
        // Property: when every row is forced to cert=false (by inflating rowE so the margin bound
        //           can never be satisfied), every returned token == 4-bit reference argmax (bit-exact).
        //
        // Method:
        //   - Inflate rowE by 1e6 (overflows f16 → inf for most values) to guarantee cert=false.
        //   - Run 50 random h rows.
        //   - Assert tokens[m] == per-row 4-bit argmax for all m.
        //
        // This directly tests the fallback kernel path (§3.2 step 5 of the spec):
        //   "fallback 行の token は文字通り既存 4-bit kernel の argmax そのもの".
        //
        // RED (stub): fails at buildLMHead2 → "not implemented".
        run("lmhead2_fallback_exact") {
            let V = 4096, K = 2048
            let wf = MLXRandom.normal([V, K]).asType(.float16)
            let (wq4, sc4, bi4opt) = MLX.quantized(wf, groupSize: 64, bits: 4, mode: .affine)
            guard let bi4 = bi4opt else { return (false, "4-bit biases nil") }
            ...
            let (wq2, sc2, bi2, realRowE) = lh2
            MLX.eval([wq2, sc2, bi2, realRowE])

            // Inflate rowE × 1e6: values above ~0.065 overflow to f16 inf, all others become huge.
            // Either way rowE[v]·‖h‖₂ ≫ any logit margin → cert=false for every row.
            let inflatedRowE = (realRowE.asType(.float32) * MLXArray(Float(1e6))).asType(.float16)
            inflatedRowE.eval()

            let M = 50
            let h = MLXRandom.normal([M, K]).asType(.float16); h.eval()

            guard let res = RawFusedVerify.lmhead2CertStep(
                x: h, w4: wq4, s4: sc4, b4: bi4,
                w2: wq2, s2: sc2, b2: bi2,
                rowE: inflatedRowE, M: M, K: K, V: V)
            else { return (false, "not implemented: lmhead2CertStep") }

            // Reference: batched 4-bit logits via qmmRows
            guard let allLogits4 = RawMetalForward.qmmRows(
                h, wq4, scales: sc4, biases: bi4, M: M, K: K, N: V)
            ...
            for m in 0..<M {
                ...
                var refTok = 0; var best = logFlat[base]
                for v in 1..<V { if logFlat[base + v] > best { best = logFlat[base + v]; refTok = v } }
                if res.tokens[m] != refTok { failures += 1 }
            }
            if failures > 0 {
                return (false, "fallback mismatch: \(failures)/\(M) tokens differ from 4-bit argmax")
            }
            let certAny = res.cert.contains(true)
            return (true, certAny
                ? "warning: some rows certified despite inflated rowE — implementation check needed"
                : "ok (\(M) fallback rows all bit-exact to 4-bit argmax)")
        }

        // Test 34 (G1-identity): lmhead2_step_identity
        //
        // Property: for M ∈ {1, 8, 17}, ALL tokens from lmhead2CertStep are byte-identical to the
        //           4-bit reference argmax — regardless of which rows are certified vs fallback.
        //
        // This is the strongest identity statement: cert rows are correct by soundness (test 32),
        // fallback rows are correct by definition (test 33); this test exercises them together with
        // mixed M shapes matching the real decode-loop call pattern (M=1 greedy, M=8 small spec batch,
        // M=17 mid-size verify).
        //
        // RED (stub): fails at buildLMHead2 → "not implemented".
        run("lmhead2_step_identity") {
            let V = 4096, K = 2048
            let wf = MLXRandom.normal([V, K]).asType(.float16)
            let (wq4, sc4, bi4opt) = MLX.quantized(wf, groupSize: 64, bits: 4, mode: .affine)
            guard let bi4 = bi4opt else { return (false, "4-bit biases nil") }
            ...
            for M in [1, 8, 17] {
                ...
                // 4-bit reference argmax via qmmRows (one GPU call, bit-exact to per-row qmm)
                guard let allLogits4 = RawMetalForward.qmmRows(
                    h, wq4, scales: sc4, biases: bi4, M: M, K: K, N: V)
                else { return (false, "ref qmmRows nil M=\(M)") }
                allLogits4.eval()
                let logFlat = allLogits4.reshaped([-1]).asType(.float32).asArray(Float.self)
                var refTokens = [Int](repeating: 0, count: M)
                for m in 0..<M {
                    let base = m * V
                    var best = logFlat[base]; var bestIdx = 0
                    for v in 1..<V { if logFlat[base + v] > best { best = logFlat[base + v]; bestIdx = v } }
                    refTokens[m] = bestIdx
                }
                ...
                guard let res = RawFusedVerify.lmhead2CertStep(
                    x: h, w4: wq4, s4: sc4, b4: bi4,
                    w2: wq2, s2: sc2, b2: bi2,
                    rowE: rowE, M: M, K: K, V: V)
                else { return (false, "not implemented: lmhead2CertStep M=\(M)") }

                for m in 0..<M {
                    if res.tokens[m] != refTokens[m] {
                        return (false,
                            "M=\(M) row=\(m): got \(res.tokens[m]) want \(refTokens[m]) " +
                            "cert=\(res.cert[m])")
                    }
                }
            }
            return (true, "ok (M=1/8/17 all tokens byte-identical to 4-bit argmax)")
        }

        // Test 35 (G1-cert-fires): lmhead2_cert_fires
        //
        // Oracle-gap closure: existing tests 32-34 use random Gaussian weights where 2-bit
        // re-quantisation error (rowE) is so large that cert NEVER fires — they pass vacuously
        // even if (a) the cert path is broken or (b) the 2-bit kernel packs weights incorrectly
        // (garbage logits2 → cert=false → fallback → still green).
        //
        // Construction (deterministic positive cert coverage):
        //   Quantise random weights to 2-bit, dequantise → the dequant result lies EXACTLY
        //   on the 2-bit affine grid. Use that as the "original" f16 weights for the 4-bit
        //   pipeline. buildLMHead2's 2-bit re-quantisation then reproduces the grid weights
        //   nearly exactly → rowE ≈ 0 → bound ≈ ε = 0.05 → cert fires on almost every row
        //   (random Gaussian h gives logit margins of O(sqrt(K)) ≈ 45 ≫ 2ε).
        //
        // Assertions:
        //   (sanity)    max(rowE) < 0.5  — confirms grid construction worked.
        //   (coverage)  certRate > 0.5   — cert must actually fire; FAIL = packing/kernel broken.
        //   (soundness) every row's token == 4-bit reference argmax (via qmmRows, bit-exact).
        //
        // Why this catches the packing bug: with rowE≈0 the bound is just ε, so if logits2
        // is garbage the argmax mismatches the 4-bit reference → assertion fails. With a
        // correct kernel, logits2 ≈ logits4 (same effective weights) → argmax matches.
        run("lmhead2_cert_fires") {
            let V = 4096, K = 2048

            // ── Step 1: build weights on a 2-bit affine grid ─────────────
            let wRaw = MLXRandom.normal([V, K]).asType(.float16); wRaw.eval()
            let (wq2raw, sc2raw, bi2rawOpt) = MLX.quantized(wRaw, groupSize: 64, bits: 2, mode: .affine)
            guard let bi2raw = bi2rawOpt else { return (false, "2-bit biases nil (grid build)") }
            MLX.eval([wq2raw, sc2raw, bi2raw])
            // Dequantise: result lies exactly on the 2-bit affine grid
            let wGrid = MLX.dequantized(wq2raw, scales: sc2raw, biases: bi2raw,
                                        groupSize: 64, bits: 2, mode: .affine).asType(.float16)
            wGrid.eval()

            // ── Step 2: quantise the grid weights to 4-bit ───────────────
            let (wq4, sc4, bi4opt) = MLX.quantized(wGrid, groupSize: 64, bits: 4, mode: .affine)
            guard let bi4 = bi4opt else { return (false, "4-bit biases nil") }
            MLX.eval([wq4, sc4, bi4])

            // ── Step 3: build lmhead2 (2-bit copy + rowE) ────────────────
            guard let lh2 = RawFusedVerify.buildLMHead2(w4: wq4, s4: sc4, b4: bi4, V: V, K: K)
            else { return (false, "not implemented: buildLMHead2") }
            let (wq2, sc2, bi2, rowE) = lh2
            MLX.eval([wq2, sc2, bi2, rowE])

            // ── Sanity: max(rowE) < 0.5 (distinguishes construction failure from kernel failure)
            let maxRowE = MLX.max(rowE).asType(.float32).item(Float.self)
            if maxRowE >= 0.5 {
                return (false,
                    "grid construction sanity FAIL: max(rowE)=\(String(format: "%.4f", maxRowE)) >= 0.5 " +
                    "(expected ~0 for 2-bit grid weights — check construction, not kernel)")
            }

            // ── Step 4: M=64 random hidden rows with generous scale ───────
            let M = 64
            let h = MLXRandom.normal([M, K]).asType(.float16); h.eval()

            // ── Step 5: run cert step ─────────────────────────────────────
            guard let res = RawFusedVerify.lmhead2CertStep(
                x: h, w4: wq4, s4: sc4, b4: bi4,
                w2: wq2, s2: sc2, b2: bi2,
                rowE: rowE, M: M, K: K, V: V)
            ...
            // ── Step 6: 4-bit reference argmax for all M rows ────────────
            guard let allLogits4 = RawMetalForward.qmmRows(
                h, wq4, scales: sc4, biases: bi4, M: M, K: K, N: V)
            else { return (false, "ref qmmRows nil") }
            allLogits4.eval()
            ...
            // ── Assertion: every row (cert and fallback alike) must match 4-bit ref ──
            for m in 0..<M {
                let base = m * V
                var refTok = 0; var best = logFlat[base]
                for v in 1..<V { if logFlat[base + v] > best { best = logFlat[base + v]; refTok = v } }
                if res.tokens[m] != refTok {
                    return (false,
                        "row=\(m): token \(res.tokens[m]) != 4-bit ref \(refTok) " +
                        "(cert=\(res.cert[m]), maxRowE=\(String(format: "%.4f", maxRowE)))")
                }
            }

            // ── Assertion: cert must fire on > half the rows ──────────────
            let certCount = res.cert.filter { $0 }.count
            let certRate  = Double(certCount) / Double(M)
            if certRate <= 0.5 {
                return (false,
                    "cert never fires — vacuous-green guard / 2-bit kernel or packing broken: " +
                    "certRate=\(String(format: "%.1f", certRate * 100))% (\(certCount)/\(M)), " +
                    "maxRowE=\(String(format: "%.4f", maxRowE))")
            }
            return (true,
                "certRate=\(String(format: "%.1f", certRate * 100))% (\(certCount)/\(M)), " +
                "maxRowE=\(String(format: "%.4f", maxRowE)), all tokens bit-exact to 4-bit ref")
        }
