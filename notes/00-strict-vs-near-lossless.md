...
作成: 2026-06-27 / 対象: Swift PoC `runHotColdHybrid`（buddy hybrid, C=64）
...
結論を先に:
- **strict lossless を保証する buddy 機構は実装されている ＝ `runHotColdSpecK`（buddy draft + exact verify の投機デコード）。** long128tok でも vs Swift-greedy **100%**。ご記憶の「buddy + M2(exact) フォールバックで lossless」はこれ。
...
`swift/Sources/QwispCore/Tell.swift` `runHotColdHybrid` の per-token accept 判定:
...
`runHotColdSpecK`（Tell.swift:398-422）が「buddy + exact フォールバックで lossless」の正体:
...
run() { QWISP_ONLY_FAST=1 QWISP_HYBRID=1 QWISP_CACHE_C=64 QWISP_PIN=48 QWISP_SKIPMODE=3 \
  ...
  QWISP_MTP_REF=/tmp/qwisp_$2_ref.safetensors $BIN stream 2>&1 | grep HotColdHybrid; }
...
run() { QWISP_ONLY_FAST=1 QWISP_SPECK=1 QWISP_CACHE_C=$1 QWISP_SKIPMODE=3 QWISP_DRAFT_K=4 \
  ...
  $BIN stream 2>&1 | grep HotColdSpecK; }
...
```bash
m0(){ QWISP_ONLY_FAST=1 QWISP_RUN_M0=1 QWISP_CACHE_C=64 QWISP_GEN=128 QWISP_SWIFT_REF=1 QWISP_MTP_REF=/tmp/qwisp_$1_ref.safetensors $BIN stream 2>&1|grep "Tell M0"; }
