...
- **実効 SSD read 帯域 + ★mmap coalescing**: 8GB streaming の最大リスク。macOS page=16KB ゆえ、1.7MB の
  expert chunk read が mmap fault で 1 回に coalesce されるか、~100 個の 16KB fault に分裂するかで実効帯域が激変
  （llama.cpp MoE streaming で確認済の現象）。**mmap-fault vs 明示 pread(1.7MB) をターゲット機で実測**して経路選択。
...
2. **mmap coalescing が最大の未知リスク**（§4）。設計 commit 前に Neo/256GB 機で直接実測すべき唯一の項目。
