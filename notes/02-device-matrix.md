...
maxK=C×3/8（C64→24, C128→48, C192→72, C256→96）は初期上限。★実効上限は **union-overflow guard** が
動的決定(2026-07-01): C×3/8 は真の安全境界でなく union~2×C で silent garbage=lossless-by-luck だったため、
実 routing の per-layer union を観測して strict-lossless 化([[status-8gb-done-16gb-next]] / notes/00)。
...
## 6. ★measured device-floor（2026-07-01, SSD-throttle + memory-pressure emulation, dev機 M1 Max 64GB）
`QWISP_SSD_THROTTLE_GBS`(preadInto leaky-bucket=単一NAND直列模擬) + memhog で低速NAND+memory pressure を
fast-SSD dev機で faithful 再現(real Metal)。全 strict-lossless(vs Swift-greedy 100%)。

**IO/token（cache thrash 指標, decode-only 実測）**: C=64→nl206/code308/mix199 MB/tok / C=128→81/31/66 / C=192→42/25/27。
= C=64 は working-set≫予算で猛 thrash(~120 expert reload/tok)、C=128+ で急減。

**tok/s（task×C×SSD-BW, 実測）**:
| C(RAM) | SSD | nl | code | mix |
|---|---|---|---|---|
| 64(8GB) | fast(par) | 18.8 | 36.7 | 45.4 |
| 64(8GB) | **1.5(Neo単一NAND)** | **3.5** | **3.9** | **5.7** |
| 128(16GB) | fast | 10.4 | 85.8 | 55.5 |
| 128(16GB) | 1.5 | 5.7 | 27.0 | 12.8 |
| 192(24GB) | fast | 9.9 | 89.9 | 89.2 |
| 192(24GB) | 1.5 | 6.9 | 30.5 | ~30 |
| 256(32GB+) | resident(IO無) | 56 | 216 | 275 |

**memory-pressure(D-native, memhog で free→60MB=Neo より厳しい)**: C=64(6.9GB)は 8GB に収まり **swap Δ=0・tok/s −7% のみ**。
→ **memory pressure は 8GB の律速でない**(tier-sizing C=64 は正しい)。full Neo 模擬(C64+1.5GB/s+pressure)= code3.7/mix5.6/nl3.9(pressure penalty ~2%=IO 支配)。

**製品結論**: 8GB は **bimodal** — fast-SSD Mac(M2/M3 Air 等)は 18-45 tok/s で良好、**$599 MacBook Neo(8GB+~1.5GB/s 単一NAND)のみ 3.7-5.6 tok/s で sub-usable**(IO律速, C=64 thrash × 低速NAND)。16GB+ は全 SSD-class で良好(fast 55-90, slow でも code 27-50)。nl は全機で forward-bound(~4-19)だが workload 10%。
**mitigation 方向(Neo tier 限定)**: C=64 の IO/tok 削減(batched verify union の縮小=accept を IO と trade), or 低速NAND runtime 検出(device-probe が SSD BW 実測済)→ その tier のみ near-lossless/短draft へ adapt。memory でなく IO を攻める。
