...
overflow prefix re-verify, commit de44e69+re-verify)。honest 値 C=64 code34/mix43/nl21, C=128 code148/mix62。
`maxK=min(QWISP_DRAFT_K, C×3/8)` は初期上限、実効は guard が動的決定。詳細 [[status-8gb-done-16gb-next]]。
