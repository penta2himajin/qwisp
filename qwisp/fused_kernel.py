...
def main():
    ...
    t_gqmm = bench(lambda: mx.gather_qmm(xe, w, s, b, rhs_indices=inds, transpose=True,
                                         group_size=gs, bits=bits, mode="affine", sorted_indices=False))
    print(f"  速度: gather_qmm(全E常駐)={t_gqmm:.3f}ms")
    for tg in (64, 128, 256):
        t = bench(lambda: bare(tg))
        print(f"        fused bare tg={tg:4}: {t:.3f}ms  ({t/t_gqmm:.1f}x gather_qmm)")
    print(f"  ※ streaming 比較対象は concat(~0.4ms)+gather_qmm(~0.45ms)=~0.85ms。fused<0.85 なら勝ち")
