...
class ExpertCache:
    def __init__(self, source, budget_per_layer: int, io_workers: int = 8):
        ...
        self.hits = 0
        self.misses = 0
        self.prefetch_hits = 0      # warm 済で miss にならずに済んだ数
        ...
        # async prefetch（背景 warm）と gather が _store/_lru を共有するので保護。
    ...
    def gather(self, layer: int, U: list[int]) -> dict[str, mx.array]:
        ...
        if miss:
            loaded = self.src.load_expert_slices(layer, miss, self._pool)  # 並列 pread（lock 外）
        with self._lock:
            for e in miss:
                if (layer, e) not in self._store:        # 背景 warm が先に入れた可能性
    ...
    def warm(self, layer: int, experts):
        """背景 prefetch: 未常駐 expert を load して store に入れる（gather を先回り）。"""
        with self._lock:
            miss = [e for e in experts if (layer, e) not in self._store]
        if not miss:
            return
        loaded = self.src.load_expert_slices(layer, miss, None)   # 呼び出し元が別スレッド
        with self._lock:
            lru = self._lru.setdefault(layer, OrderedDict())
            for e in miss:
                if (layer, e) not in self._store:
                    self._store[(layer, e)] = loaded[e]
                    lru[e] = None
                    lru.move_to_end(e)
