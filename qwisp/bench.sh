...
rm -f "$TMP"
echo "-- axes: fidelity=teacher-forced per-token vs strict (primary). correctness=L3 acceptability:"
echo "   read as DELTA (strict PASS -> bolt PASS = divergence is benign); absolute is confounded by"
echo "   base-model capability (e.g. agentic FAILs for strict too), so treat such hooks as reference. --"
