...
- NEW LEVER FOUND by the provenance audit (qwisp `e8a734c` + Lean `b_exact_wins`): B-EXACT register qmv variants (x_thread[NB][16] sized exactly — the old rows_b NO-GO had measured a fixed [4][16] array) are BIT-EXACT and beat per-row qmv at B=2 (−8%) / B=3 (−8..−10%), tie at B=4. Projected end-to-end ~+1-2 tok/s at B=3 if wired through the M=B projection dispatches (optional, similar size to Stage-1b tax; combined maybe +3-4). Remaining distance to the 3083 physics ceiling = occupancy (576/512/448 vs 1024 maxThreads/tg) + per-row dequant compute; next design lever = shrink per-row register cost (half-storage x_thread; subnormal-exactness caveat on /4096).
