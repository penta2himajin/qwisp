...
- Lean-certified (qwisp-lean `a0f3247`, `Qwispmath/LaneCeiling.lean`, no sorry): NO-GO ratios (tiled ≥7x M≤4/≥6x M=8; rows_b ≥3.9x), Stage-1b recovery < +2 tok/s, raw≡MLX marginal bands, aggregate ceilings (233 empirical / 794 physics), W+B·S floor per lane on T4. Provenance-audited (§0 of the file): batching efficiency is MODEL-DEPENDENT — 82-83% vs the pure-BW ideal, 68-69% vs the two-component (latency+BW) ideal; honest range [~0.69, ~0.83], robust to ±3% measurement error (0.78-0.89 envelope), both conservative w.r.t. the routed-MoE union effect. The NO-GO verdicts do not depend on the ideal choice.
