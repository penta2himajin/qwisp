...
Env overrides: MARGIN_THR=3 MISS_THR=0.20 BIN=10 BURST=4
"""
import os, sys
...
def analyze(path):
    ...
    dec = []  # decode rows only (M==1): (tok, miss_rate, margin, both_bad)
    for r in rows:
        ...
        if M != 1 or routed == 0:
            continue
        mr = miss / routed
        both = margin < MARGIN_THR and mr > MISS_THR
        dec.append((tok, mr, margin, both))
    if not dec:
        print(f"{path}: no M==1 decode rows")
        return
    n = len(dec)
    # per-third both-bad rate
    thirds = [dec[: n // 3], dec[n // 3 : 2 * n // 3], dec[2 * n // 3 :]]
    tr = [sum(x[3] for x in t) / len(t) * 100 if t else 0 for t in thirds]
    # burst bins
    burst_bins = []
    for i in range(0, n - BIN + 1, BIN):
        c = sum(x[3] for x in dec[i : i + BIN])
        if c >= BURST:
            burst_bins.append((dec[i][0], c))
    print(f"{path}: decode_steps={n}  both-bad thirds "
          f"[{tr[0]:.0f}% {tr[1]:.0f}% {tr[2]:.0f}%]  "
          f"burst_bins(>={BURST}/{BIN})={len(burst_bins)}")
    for tok, c in burst_bins:
        print(f"    burst @ tok~{tok}: {c}/{BIN} both-bad")
