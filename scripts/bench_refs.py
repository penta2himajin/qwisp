...
Reproducible: prompts live in scripts/bench_prompts.py (committed). Generated refs are written to
...
After generating, replace spec_greedy via --ingest-swift with a raw-spec dump:
  ...
  PYTHONPATH=<repo> "$PY" -m scripts.bench_refs --ingest-swift code /tmp/code.toks
...
Run (MTPLX runtime venv has mlx_lm):
  ...
  PYTHONPATH=<repo> "$PY" -m scripts.bench_refs <model_dir> [--nspec 128]
"""
...
import scripts.mtp_decode as MD
from scripts.bench_prompts import PROMPTS
